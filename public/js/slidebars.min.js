...
$(document).ready(function() {
	...
	$.slidebars = function(options) {
		...
		var ua = navigator.userAgent, // Get users agent string.
		android = false, // Variable for storing android version.
		iOS = false; // Variable for storing iOS version.
		
		if (/Android/.test(ua)) { // Detect Android in users agent string.
			android = ua.substr(ua.indexOf('Android')+8, 3); // Set version of Android.
		} else if (/(iPhone|iPod|iPad)/.test(ua)) { // Detect iOS in users agent string.
		...
		function css() {
			...
			if ($left && $left.hasClass('sb-width-custom')) $left.css('width', $left.attr('data-sb-width')); // Set users custom width.
			if ($right && $right.hasClass('sb-width-custom')) $right.css('width', $right.attr('data-sb-width')); // Set users custom width.
		...
		function animate(object, amount, side) {
			...
			if (object.hasClass('sb-style-push')) {
				selector = $site.add(object).add($slide); // Push - Animate site, Slidebar and users elements.
			} else if (object.hasClass('sb-style-overlay')) {
				selector = object; // Overlay - Animate Slidebar only.
			} else {
				selector = $site.add($slide); // Reveal - Animate site and users elements.
			}
		...
		this.open = open; // Maps users variable name to the open method.
		this.close = close; // Maps users variable name to the close method.
		this.toggle = toggle; // Maps users variable name to the toggle method.
